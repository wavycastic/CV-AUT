...
namespace CvAut
{
    ...
    {
        private enum DropSide { TopLeft, BottomLeft, BottomRight, TopRight }

        private sealed record ExternalAreaModel(Point Left, Point Right, Point Top, Point Bottom)
        {
        ...
        private BuilderBaseDropPlanner BuildDropPlanner()
        {
            ...
            BuilderBaseDropPlanner planner = BuilderBaseDropPlanner.Build(screenshot, ScreenWidth, ScreenHeight);
        ...
        private static Point AvoidPotionArea(Point point)
        {
            int potionCapY = 500 + Math.Max(0, ScreenHeight - 732);
        ...
        private static void SaveDropDebugOverlay(Mat? screenshot, BuilderBaseDropPlanner planner)
        {
            ...
            try
            {
                using Mat canvas = screenshot == null || screenshot.Empty() ? new Mat(ScreenHeight, ScreenWidth, MatType.CV_8UC3, Scalar.Black) : screenshot.Clone();
        ...
        private sealed class BuilderBaseDropPlanner
        {
            ...
            public static BuilderBaseDropPlanner Build(Mat? screenshot, int screenWidth, int screenHeight)
            {
                ...
                foreach (DropSide side in Enum.GetValues<DropSide>())
                {
                    ...
                    dropLines[side] = MakeDropLine(baseLine, side).Select(p => OffsetOutside(p, side, nearOffset, screenHeight)).Select(AvoidPotionArea).Where(IsSafeDropPoint).ToList();
                    furtherDropLines[side] = MakeDropLine(baseLine, side).Select(p => OffsetOutside(p, side, farOffset, screenHeight)).Select(AvoidPotionArea).Where(IsSafeDropPoint).ToList();
                }
            ...
            private static List<Point> MakeDropPoints(IReadOnlyList<Point> vector, DropSide side, int pointsQty, int addTiles, Random random, int screenHeight)
            {
                ...
                for (int i = p - 1; i < vector.Count; i += p)
                {
                    ...
                    output.Add(AvoidPotionArea(ClampBottom(adjusted, screenHeight)));
                }
            ...
            private static List<Point> FallbackOuterGreenLine(DropSide side)
            {
                ...
                {
                    DropSide.TopLeft => TopLeftDrop,
                    DropSide.TopRight => TopRightDrop,
                    DropSide.BottomLeft => BottomLeftDrop,
                    _ => BottomRightDrop
                };

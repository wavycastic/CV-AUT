...
internal sealed class FarmingService
{
    ...
    public bool BattleEnded(out string matchInfo)
    {
        ...
        bool hasContinue = TryFindContinueButton(screenshot, out Point center, out double continueScore);
            bool hasResultMarker = _vision.TryFindTemplateRegion(
            screenshot, @"ui\resources_gained.png", AutomationRoiConstants.ResultYouGotRoi, AutomationRoiConstants.ResultYouGotRoi,
            AutomationThresholds.ResultYouGotThreshold, out _, out double markerScore);
